...
const Quanlycongty_index = () => {
  ...
  return (
    <div className="absolute overflow-hidden flex flex-col top-0 left-0 w-screen z-100 bg-white h-screen">
      <div className="flex flex-col flex-1 overflow-hidden bg-[#e6e7ed]">
        <Link
          to="/mobile/canhan"
          className="flex min-h-14 h-14 text-[#2d2d38] items-center gap-4 px-4 
          shadow border-b border-[#0003] bg-[white]"
        >
          <FaArrowLeft /> Quản lý công ty
        </Link>
        <div className="flex flex-col gap-2 overflow-hidden p-4 pt-2 flex-1">
          {user?.profile?.level !== "admin" ? (
            <div className="flex p-8 justify-center">Bạn không có quyền!</div>
          ) : (
            <div className="flex justify-end">
              <Themmoi_congty>
                <Button icon={<FaPlus />} type="primary"></Button>
              </Themmoi_congty>
            </div>
          )}
          <div className="flex flex-col gap-2 overflow-y-auto h-full fadeInTop pb-22">
            {init?.companies
              ?.sort((a, b) => b?.id - a?.id)
              ?.map((comp) => (
